...
class LinkedListNode(BaseModel):
    ...
    prev_id: Optional[int] = None  # For doubly linked lists
    color: str = Field(default="default")
    highlighted: bool = Field(default=False)


class LinkedListState(BaseModel):
    """State of a linked list"""
    name: str = Field(..., min_length=1, max_length=50)
    type: str = Field(..., description="Type: singly, doubly, circular")
    nodes: List[LinkedListNode] = Field(default_factory=list, max_items=1000)
    head_id: Optional[int] = None
    tail_id: Optional[int] = None
    
    @validator('nodes')
    def validate_node_references(cls, v, values):
        """Ensure next_id and prev_id reference valid nodes"""
        if not v:
            return v
        
        node_ids = {node.id for node in v}
        for node in v:
            if node.next_id is not None and node.next_id not in node_ids:
                raise ValueError(f'Node {node.id} references invalid next_id {node.next_id}')
            if node.prev_id is not None and node.prev_id not in node_ids:
                raise ValueError(f'Node {node.id} references invalid prev_id {node.prev_id}')
...
class VisualizationFrame(BaseModel):
    ...
    linked_lists: List[LinkedListState] = Field(default_factory=list, max_items=10)
    ...
    def at_least_one_structure(self):
        ...
        structures = ['arrays', 'variables', 'trees', 'graphs', 'linked_lists', 'stacks', 'queues']
...
class MetadataModel(BaseModel):
    ...
    total_frames: int = Field(..., ge=1, le=50)  # CHANGED: Max 50 frames
...
class VisualizationData(BaseModel):
    ...
    frames: List[VisualizationFrame] = Field(..., min_items=1, max_items=50)  # CHANGED: Max 50 frames
